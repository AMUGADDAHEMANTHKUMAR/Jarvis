...
if __name__ == "__main__":
    print("=" * 50)
    print("JARVIS IS ONLINE")
    print("Say '1' or 'Jarvis' → voice mode")
    print("Say '2' or 'text'   → text mode")
    print("Say '3' or 'exit'   → shut down")
    print("=" * 50)
    ...
    while True:
        try:
            ...
            if not wake_text:
                continue
            if any(w in wake_text for w in ["three", "3", "exit", "stop"]):
                speak("Goodbye.")
                quit()
            elif any(w in wake_text for w in ["two", "2", "text"]):
                mode = "text"
                ...
                print("\n[Jarvis AWAKE — TEXT mode]")
                break
            elif any(w in wake_text for w in ["one", "1", "jarvis"]):
                mode = "voice"
                speak("Jarvis online. Voice mode. I am listening.")
                print("\n[Jarvis AWAKE — VOICE mode]")
                break
    ...
    while True:
        try:
            if mode == "voice":
                ...
                print("\n[Text Mode] Type command:")
                print("  (type 'voice' or '1' to switch, 'exit' to stop)")
                command = input(">> ").strip()
                if not command:
                    continue
                command_lower = command.lower()
                if any(w in command_lower for w in ["exit", "stop", "shutdown"]):
                    speak("Shutting down. Goodbye.")
                    break
                if command_lower in ["voice", "1", "speak"]:
                    mode = "voice"
                    speak("Voice mode. I am listening.")
                    print("\n[VOICE mode]")
                    continue
                run_command(command)
